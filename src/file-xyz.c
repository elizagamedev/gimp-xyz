...
const char LOAD_PROCEDURE[] = "file-xyz-load";
const char SAVE_PROCEDURE[] = "file-xyz-save";
const char BINARY_NAME[]    = "file-xyz";
...
void query()
{
    ...
    gimp_install_procedure(LOAD_PROCEDURE,
                           "Loads images in RPGMaker XYZ format",
                           "Loads images in RPGMaker XYZ format",
                           "Mathew Velasquez",
                           "Mathew Velasquez",
                           "2013",
    ...
    gimp_install_procedure(SAVE_PROCEDURE,
                           "Saves images in RPGMaker XYZ format",
                           "Saves images in RPGMaker XYZ format",
                           "Mathew Velasquez",
                           "Mathew Velasquez",
                           "2013",
    ...
    gimp_register_file_handler_mime(LOAD_PROCEDURE, "image/xyz");
    gimp_register_load_handler(LOAD_PROCEDURE, "xyz", "");

    // Now register the save handlers
    gimp_register_file_handler_mime(SAVE_PROCEDURE, "image/xyz");
    gimp_register_save_handler(SAVE_PROCEDURE, "xyz", "");
}
         ...
         GimpParam **return_vals)
{
    ...
    *return_vals  = return_values;
    ...
    if (!strcmp(name, LOAD_PROCEDURE)) {
        ...
        // Now read the image
        new_image_id = read_xyz(param[1].data.d_string);
        ...
        *nreturn_vals = 2;
        ...
        case GIMP_EXPORT_IGNORE:
            ...
            status = write_xyz(param[3].data.d_string, image_id,
                               drawable_id);
